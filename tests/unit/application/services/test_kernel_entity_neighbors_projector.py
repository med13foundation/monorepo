"""Unit tests for the entity-neighbors projector."""
...
def _seed_space(db_session) -> ResearchSpaceModel:
    user = UserModel(
        email=f"neighbors-{uuid4().hex}@example.com",
        username=f"neighbors-{uuid4().hex[:8]}",
        full_name="Neighbors Tester",
    ...
    space = ResearchSpaceModel(
        slug=f"entity-neighbors-{uuid4().hex[:12]}",
        name="Entity Neighbors Space",
        description="Unit test space for entity neighbors projector",
        owner_id=user.id,
        status="active",
    )
...
def _seed_dictionary(db_session) -> tuple[str, str, str]:
    ...
    gene_type = f"NB_GENE_{suffix}"
    phenotype_type = f"NB_PHENOTYPE_{suffix}"
    relation_type = f"NB_ASSOCIATED_WITH_{suffix}"
...
def _seed_relation_bundle(
    ...
    *,
    ...
    claim = RelationClaimModel(
        ...
        polarity="SUPPORT",
        claim_text="Projected support claim.",
...
def test_rebuild_populates_entity_neighbors(db_session) -> None:
    ...
    _seed_relation_bundle(
        db_session,
        space_id=space.id,
        source_id=target_b.id,
        target_id=source.id,
        relation_type=relation_type,
        target_type=gene_type,
    )

    projector = KernelEntityNeighborsProjector(db_session)

    rebuilt_rows = projector.rebuild(space_id=str(space.id))

    assert rebuilt_rows == 4
    source_rows = list(
        db_session.query(EntityNeighborModel)
        .filter(EntityNeighborModel.entity_id == source.id)
        .order_by(EntityNeighborModel.direction.asc())
        .all(),
    )
    assert len(source_rows) == 2
    assert {row.neighbor_entity_id for row in source_rows} == {target_a.id, target_b.id}
    assert {row.direction for row in source_rows} == {"incoming", "outgoing"}


def test_apply_update_removes_deleted_neighbor_rows(db_session) -> None:
    gene_type, phenotype_type, relation_type = _seed_dictionary(db_session)
    space = _seed_space(db_session)
    source = EntityModel(
        research_space_id=space.id,
    ...
    relation_id = _seed_relation_bundle(
        db_session,
        space_id=space.id,
        source_id=source.id,
        target_id=target.id,
    ...
    assert updated_rows in {0, 2}
    assert (
        db_session.get(
            EntityNeighborModel,
            {"entity_id": source.id, "relation_id": relation_id},
        )
        is None
    )
    assert (
        db_session.get(
            EntityNeighborModel,
            {"entity_id": target.id, "relation_id": relation_id},
        )
        is None
    )
