"""Unit tests for the entity-claim summary rebuild script."""
...
from scripts import rebuild_graph_read_model_entity_claim_summary as rebuild_script
...
def test_run_rebuild_for_single_space(capsys) -> None:
    ...
    projector = _ProjectorStub(rebuilt_rows=5, calls=[])
    ...
    assert "model=entity_claim_summary" in capsys.readouterr().out


def test_run_rebuild_globally_can_emit_json(capsys) -> None:
    session = _SessionStub()
    projector = _ProjectorStub(rebuilt_rows=8, calls=[])
    ...
    assert '"model_name": "entity_claim_summary"' in output
    assert '"rebuilt_rows": 8' in output
