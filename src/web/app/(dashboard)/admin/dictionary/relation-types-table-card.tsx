...
import type { DictionaryRelationTypeListResponse } from '@/types/dictionary'

interface RelationTypesTableCardProps {
  relationTypes: DictionaryRelationTypeListResponse | null
  error?: string | null
}

export function RelationTypesTableCard({
  relationTypes,
  error,
}: RelationTypesTableCardProps) {
  const rows = relationTypes?.relation_types ?? []
            ...
            <TableBody>
              {rows.map((row) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell>{row.is_directional ? 'Yes' : 'No'}</TableCell>
                  <TableCell>
