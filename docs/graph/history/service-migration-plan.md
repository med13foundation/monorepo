...
- [x] Restore `make graph-service-checks` to green after the stale deleted-path reference in the graph-service lint/type-check target list is removed
- [ ] Run deployed shared-instance topology validation cleanly in production
- [x] Reduce repo-global packaging/migration coupling so graph-service container and migration ownership are less dependent on root `src/` and root Alembic assets
...
- 2026-03-13: Phase 8 advanced again: `make graph-service-checks` is green again after removing the stale deleted-path reference, graph-service Alembic config/assets now live under `services/graph_api/alembic.ini` and `services/graph_api/alembic/`, and repo/test migration entrypoints now execute through that service-local config. The only remaining Phase 8 item is the first successful real deployed shared-instance topology-validation run.
- 2026-03-13: Phase 6 advanced substantially: graph-specific search/connection runtime construction now delegates through `services/graph_api/composition.py`, but hypothesis generation still needs to be rebuilt from service-local composition instead of the legacy platform container.
- 2026-03-13: Phase 9 advanced substantially: active graph docs now publish only the standalone `/v1/...` graph-service contract, but the remaining service-package imports from platform code still need to be removed before legacy removal is complete.
...
- The current standalone-service release gate is partially green: `scripts/validate_graph_service_boundary.py` and the graph Postgres/performance suites pass, but `make graph-service-checks` still fails because the graph-service lint/type path list references a removed file.
- Even after the standalone gate is restored, packaging and migration ownership remain partly repo-global because the graph-service container and migration command still package/use root `src/` and root Alembic assets.
- After those two repo-level issues are addressed, the final remaining migration item will be the first successful real deployed shared-instance topology-validation run.
