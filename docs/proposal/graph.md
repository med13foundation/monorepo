...
- Remaining: hard-enforce orphan-relation rejection, move canonical write ownership fully behind a projection service, and tighten read paths so canonical graph responses are guaranteed claim-backed by policy rather than convention.
...
[ ] Add migration checks and staged enforcement for orphan canonical relations so new writes cannot create a `relations` row without projection lineage, while existing orphan rows are reported and remediated through a backfill or curator repair workflow before full enforcement.
